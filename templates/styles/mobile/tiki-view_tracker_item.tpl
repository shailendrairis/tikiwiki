{* $Id: tiki-view_tracker_item.tpl 48799 2013-11-28 21:39:34Z xavidp $ *}
...
{if ! isset($print_page) || $print_page ne 'y'}
	...
	<div class="navbar" align="right" data-role="controlgroup" data-type="horizontal">{* mobile *}
		{if $prefs.feature_group_watches eq 'y' and ( $tiki_p_admin_users eq 'y' or $tiki_p_admin eq 'y' )}
			<a data-role="button" href="tiki-object_watches.php?objectId={$itemId|escape:"url"}&amp;watch_event=tracker_item_modified&amp;objectType=tracker+{$trackerId}&amp;objectName={$tracker_info.name|escape:"url"}&amp;objectHref={'tiki-view_tracker_item.php?trackerId='|cat:$trackerId|cat:'&itemId='|cat:$itemId|escape:"url"}" class="icon">{icon _id='eye_group' alt="{tr}Group Monitor{/tr}" align='right' hspace='1'}</a>
		{/if}
		{if $prefs.feature_user_watches eq 'y' and $tiki_p_watch_trackers eq 'y'}
			{if $user_watching_tracker ne 'y'}
				<a data-role="button" href="tiki-view_tracker_item.php?trackerId={$trackerId}&amp;itemId={$itemId}&amp;watch=add" title="{tr}Monitor{/tr}">{icon _id='eye' align="right" hspace="1" alt="{tr}Monitor{/tr}"}</a>
			{else}
				<a data-role="button" href="tiki-view_tracker_item.php?trackerId={$trackerId}&amp;itemId={$itemId}&amp;watch=stop" title="{tr}Stop Monitor{/tr}">{icon _id='no_eye' align="right" hspace="1" alt="{tr}Stop Monitor{/tr}"}</a>
			{/if}
		{/if}
			<a data-role="button" class="link" href="tiki-view_tracker_item.php?trackerId={$trackerId}&amp;itemId={$itemId}&amp;print=y" title="{tr}Print{/tr}">{icon _id='printer' align='right' hspace='1' alt="{tr}Print{/tr}"}</a>
		{if $item_info.logs.cant}
			<a data-role="button" class="link" href="tiki-tracker_view_history.php?itemId={$itemId}" title="{tr}History{/tr}">{icon _id='database' align='right' alt="{tr}History{/tr}"}</a>
		{/if}
	</div>
								...
								{if $canView}
									...
									{if $canRemove}
										<div data-role="controlgroup" data-type="horizontal">{* mobile *}
											<a data-role="button" class="link" href="tiki-view_tracker.php?trackerId={$trackerId}&amp;remove={$itemId}" title="{tr}Delete{/tr}">{icon _id='cross' alt="{tr}Delete{/tr}"}</a>
										</div>
									{/if}
								{/if}
							{/if}
				...
				<tr>
					<td colspan="2">
						...
						<div data-role="controlgroup" data-type="horizontal">{* mobile *}
							...
							{/if}
							{if $item_info.logs.cant}
								<a data-role="button" class="link" href="tiki-tracker_view_history.php?itemId={$itemId}" title="{tr}History{/tr}">{icon _id='database' alt="{tr}History{/tr}"}</a>
							{/if}
							{if $tiki_p_admin_trackers eq 'y' && empty($trackers)}
								<a data-role="button" class="link" href="tiki-view_tracker_item.php?itemId={$itemId}&moveto" title="{tr}Move to another tracker{/tr}">{icon _id='arrow_right' alt="{tr}Move to another tracker{/tr}"}</a>
							{/if}
						</div>
					</td>
				</tr>
