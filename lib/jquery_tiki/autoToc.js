// $Id: autoToc.js 48781 2013-11-27 23:54:26Z jobicarter $
/*
file: autoToc.js
author: Jobi Carter
...
$.buildAutoToc = function () {
	...
	if (document.getElementById("page-data") && !document.getElementById("toc-off") && document.location.href.indexOf("tiki-print.php") == -1) {
		...
		if (headers.length > 1) {
			...
			tocString = "<div id='outerToc' contenteditable='false'> <div id='outerTocTitle'> <a href='#'> Table of Contents, or Click for Page Top</a></div><div id='innerToc' contenteditable='false'></div></div><div id='outerToc-static' contenteditable='false'><div id='toc-static'></div></div>";
			...
			//make it so when you scroll past the bottom of the overflow it doesn't scroll the page, making the autoToc dissapear
			

			
			//set mouse scroll on innerToc to not scroll main page when hit bottom of innerToc scroll
			 if (navigator.appName == 'Microsoft Internet Explorer')
                        {
			//need to find a solution for IE.
                                ...
                                document.getElementById('innerToc').onmousewheel = function(e) {
                                        document.getElementById('innerToc').scrollTop -= e.wheelDeltaY;
                                        preventDefault(e);
                                }
                        }
			...
			$.event.add(window, "scroll", function () {
				...
				if (p > start) {
					...
					var elem = document.getElementById('outerToc');
					if (elem) {
