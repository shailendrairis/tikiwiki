/* $Id: tiki-toolbars.js 48104 2013-10-20 19:09:38Z arildb $
	...
	$("#tbOLinkObjectType").change(function () {
		$("#tbOLinkObjectSelector").object_selector(
			...
			function($option, value) {
				var str, selected;	// will only select an option if in the first 25 on the list
				if (value.object_type === 'wiki page') {
					str = "((" + value.url.replace(/\+/g, " ") + "|" + value.title + "))";
					selected = url === value.url.replace(/\+/g, " ");
				} else {
					str = "[" + value.url + "|" + value.title + "]";
					selected = url === value.url;
				}
				$option.val(str).text(value.title).prop("selected", selected);
				return $option;
			}
		);
	}).change();
...
function dialogObjectLinkInsert( area_id, dialog ) {
	var url = $("#tbOLinkObjectSelector").val(), linkDesc = $("#tbOLinkDesc");
	if (!url) {
